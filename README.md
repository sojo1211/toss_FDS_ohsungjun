...
저는 **"사고를 탐지하는 것보다 사전에 예방하는 것이 리스크 관리의 본질"**이라는 신념을 가지고 있습니다. 이러한 문제의식은 스테이블코인의 실시간 건전성을 평가하는 **SRHS(Stablecoin Risk Health Score)** 정량화 연구로 이어졌고, **학술적 성과(KCI 등재 및 최우수 논문상)를 통해 리스크 탐지력과 오탐률(False Positive) 사이의 최적 임곗값(Threshold)을 설계하는 금융 모델링 역량**을 다졌습니다.

이후 이론적 연구를 넘어 실제 금융 서비스가 작동하는 현장을 체득하기 위해 **Toss CX 리스크 운영 부서**에 합류했습니다. 실무 현장에서 매일 쏟아지는 RMS Alert를 판독하고 가맹점 소명 자료와 재무 상태 등의 내부 데이터를 교차 검증하며, 자동화된 시스템의 빈틈을 메우는 **주도적이고 꼼꼼한 리스크 검증 역량**을 키웠습니다. 나아가, 현장 판독 효율성을 높이기 위해 **오탐 감소 복합 룰과 테마 연계 모니터링 규칙** 등을 분석해 팀에 제안하기도 했습니다.

이제 저는 이 두 가지 경험—**정량적 리스크 모델 설계 능력**과 **현장의 꼼꼼한 모니터링 실무 감각**—을 하나로 결합하고자 합니다. 토스뱅크의 Anti-Fraud Manager로서, 금융 범죄 예방을 위한 FDS 예방 룰 고도화와 유저 이상거래 데이터 분석에 기여하며 가장 안전한 금융 생태계를 구축하는 데 기여하고자 합니다.
