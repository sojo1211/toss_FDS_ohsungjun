...
저는 AI 기술을 통해 사회적 문제를 예방하는 일에 깊은 열정을 쏟아왔습니다. 대학 시절 낙상 사고 전조 예측(SafeFall)과 공급망 리스크 분석을 수행하며 **“사고가 발생한 뒤 감지하는 것보다, 데이터 속 숨겨진 전조 신호를 포착하여 사전에 예방하는 것이 본질적인 해결책”**이라는 강력한 신념을 얻었습니다. 이 신념을 금융 현장에서 직접 검증하고 체득하기 위해 **토스씨엑스 FDS Team의 Operations Supporter(리스크운영)**로 합류했습니다.

매일 수천 건의 가맹점 데이터를 들여다보며, 서류상 등록된 입점 팩터와 실제 운영되는 데이터 사이의 정교한 갭(Gap)을 추적하고, 14대 RMS 기준에 따라 리스크 사유를 정량 라벨링해왔습니다. 미소명 및 고위험 가맹점에 대해 지급보류나 결제 OFF 조치를 발송하고 집행하는 과정에서 뼈저리게 느낀 것은, **“아무리 뛰어난 AI 모델과 자동화 FDS 시스템이 존재하더라도, 실무 데이터의 미세한 틈새와 예외 케이스를 현장에서 잡아내지 못하면 결국 자금 유출과 금융 사고라는 거대한 댐의 균열로 이어진다”**는 사실이었습니다. 

이러한 현장에서의 생생한 리스크 운영 감각은 저에게 데이터 이면의 리스크를 역추적하는 깊은 통찰력을 선물해주었습니다. 이제 저는 현장에서 목격한 **'실무형 리스크 운영 감각(Operations)'**과 학술적/기술적으로 쌓아 올린 **'Python/ML 기반의 데이터 Deep Dive 분석 기술(Quantification)'**을 융합하여, 한 단계 더 나아간 방어선을 구축하고자 합니다.

토스뱅크의 **Anti-Fraud Manager**로서, 비대면 여신 상품 및 신규 가입 신청 시점부터 도사리고 있는 Fraud 위험 요소를 선제적으로 분석하고 예방 Rule을 정교하게 고도화하겠습니다. 리스크를 사전에 예측하여 사용자와 토스뱅크 모두를 안심하게 만드는 가장 튼튼한 금융 안전망의 주역이 되는 것이 저의 최종 목표입니다.
